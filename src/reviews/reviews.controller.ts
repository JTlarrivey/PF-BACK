...
export class ReviewsController {
  ...
  @Get('/books/:book_id')
  @UseGuards(AuthGuard, UserStatusGuard)
  async getReviewsByBook(@Param('book_id') book_id: string) { // <-- Recibe como string
    try {
      const reviews = await this.reviewsService.getReviewsByBook(Number(book_id)); // <-- Convierte a número
      if (reviews.length === 0) {
